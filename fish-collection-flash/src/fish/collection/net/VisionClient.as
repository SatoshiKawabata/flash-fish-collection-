package fish.collection.net
{
	 ...
	 * @author atsumo
	 */
	public class VisionClient implements WebSocketHandler
	{
		// MainModel
		...
		private function openNext():void
		{
			...
			if (_tryCount >= _url.length) {
				...
				_model.showError(error, true);
				return;
			}
			_websocket = new VisionClientWebSocket(this);
			_websocket.open(_url[_tryCount], _origin);
			// 試行カウントを１増やしておく
			_tryCount++;
		}
		 ...
		 */
		public function addDelegate(delegate:IVisionClientDelegate):void
		{
...
//			}
		...
		{
			//Loading.block('loading.disconnected');
			// ハートビートタイマー停止
		...
		private function _showError(error:Error):void
		{
			//Loading.unblock();
			_model.showError(error);
		}
